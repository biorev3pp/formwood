        ...
        <div class="row breadcrumbs-top">
            <div class="breadcrumb-wrapper pl-1">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{route('floorplan')}}">Floorplans</a>
                    </li>
                    <li class="breadcrumb-item"><a href="{{route('floorplan-elevation', ['elevation_id' => base64_encode($elevation_id)])}}">Exterior</a>
                    </li>
        ...
        <div class="col-xl-3 col-lg-4 col-sm-6">
            <div class="card" id="card{{$floorplan->id}}">
                ...
                <div class="card-footer border-top-blue-grey border-top-lighten-5 text-muted">
                    ...
                    <a href="javascript:;" onclick="FloorPlanModal(true, '{{$floorplan->title}}', '{{$floorplan->status_id}}', '{{$floorplan->image}}',{{$floorplan->id}}, '{{$floorplan->thumbnail}}')" data-toggle="tooltip" title="Edit Homeplan" class="text-dark mr-25 edit-button"> <i class="ft-edit"></i> </a>
            ...
            <form id="designForm">
                <div class="modal-body">
                    ...
                    <div class="form-group d-flex flex-wrap justify-content-start">
                        ...
                        <div class="mr-2">
                            <label for="thumbnailImage" class="text-uppercase mb-1">Thumbnail/Icon</label>
                            <figure class="position-relative w-150 mb-0">
                                <img src="{{asset('media/placeholder.jpg')}}" class="img-thumbnail">
                                <input type="file" id="thumbnailImage" class="d-none" onchange="readUrlThumb(this,'thumbnail')">
...
<script>
    ...
    function FloorPlanModal(...values){
        ...
        if(values[0] == true){
            ...
            if(values[5] != ""){
                modal.find('#thumbnailImage').prev().attr('src', `${pathThumbnail}/${values[5]}`);
            }
    ...
    const editImageValidationThumb = () => {
        if(isChangeThumb == 'thumbnail'){
            if(thumbnailImage == null){
                toastr.clear()
                toastr.error('View 1 Base Image is required');
                return false;
    ...
    function submitForm(editable){
        ...
        if(editable == true){
            ...
            if(editImageValidationThumb() == false){
                return false;
            }
        }
        else{
        ...
        if(editable == true){
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    parent.find('.edit-button').attr('onclick', `FloorPlanModal(true, '${response.title}', '${response.status_id}', '${response.image}', ${response.id}, '${response.thumbnail}')`);
                    ...
                    card = `<div class="col-xl-3 col-lg-4 col-sm-6">
                                <div class="card" id="card${response.id}">
                                    ...
                                    <div class="card-footer border-top-blue-grey border-top-lighten-5 text-muted">
                                        ...
                                        <span class="float-right">
                                        <a href="{{route('homeplan-features',['floorplan_id' => base64_encode($floorplan->id)])}}" data-toggle="tooltip" title="View Features" class="text-dark mr-25"> <i class="ft-eye"></i> </a>
                                            <a href="javascript:;" onclick="FloorPlanModal(true, '${response.title}', '${response.status_id}', '${response.image}',${response.id}, '${response.thumbnail}')" data-toggle="tooltip" title="Edit Floorplan" class="text-dark mr-25 edit-button"> <i class="ft-edit"></i> </a>
    ...
    function readUrlThumb(input, element) {
        if (input.files && input.files[0]) 
        {  
            var reader = new FileReader();
            reader.onload = function (e) 
